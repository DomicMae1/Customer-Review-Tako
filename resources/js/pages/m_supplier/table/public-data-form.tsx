...
import { Attachment, AttachmentType, MasterSupplier } from '@/types';
...
export default function PublicSupplierForm({
    supplier,
    onSuccess,
    isFilled = false,
}: {
    ...
    const { supplier_name, token, user_id, id_perusahaan, company, attachmentRules } = usePage().props as unknown as {
    ...
    async function uploadAttachment(file: File, type: AttachmentType, npwpNumber: string): Promise<Attachment> {
        ...
        const res = await axios.post('/supplier/upload-temp', formData);
    ...
    const submitFinalData = async () => {
        ...
        try {
            const res = await axios.post(route('supplier.public.submit'), pendingPayload);
    ...
    const handleSubmit: FormEventHandler = async (e) => {
        ...
        const getFileStatus = (newFile: UploadedFileState | null, type: string) => {
            ...
            return supplier?.attachments?.some((a) => a.type === type);
        };
        ...
        if (data.jenis_perusahaan !== 'Perusahaan Luar Negeri' && rules.is_nib && !isSupplierPerorangan && !getFileStatus(nibFile, 'nib')) {
        ...
        const prepareAttachment = (fileState: UploadedFileState | null, type: string) => {
            ...
            const existing = supplier?.attachments?.find((a) => a.type === type);
            if (existing) {
        ...
        const rawAttachments = [
            ...
            isSupplierPerorangan ? null : prepareAttachment(nibFile, 'nib'),
    ...
    return (
        <>
                ...
                <div className="relative">
                    ...
                    <div className="mx-auto max-w-7xl rounded-2xl p-4 xl:border 2xl:mt-0">
                        <div className="mb-6 flex items-start justify-between gap-4">
                            <h1 className="min-w-0 text-2xl font-semibold sm:text-3xl">{supplier ? 'Edit Data Supplier' : 'Data Supplier'}</h1>
                                    ...
                                    <div className="col-span-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                                        ...
                                        <div className="w-full">
                                            <ResettableDropzone
                                                ...
                                                uploadConfig={{
                                                    url: '/supplier/upload-temp', // Endpoint upload raw
                                                    payload: {
                                                        type: 'npwp',
                                                        npwp_number: data.no_npwp,
                                                        id_perusahaan: id_perusahaan,
                                                ...
                                                existingFile={supplier?.attachments?.find((a) => a.type === 'npwp')}
                                            />
                                        ...
                                        <div className="w-full">
                                            <ResettableDropzone
                                                label="Upload NIB"
                                                isRequired={rules.is_nib && data.bentuk_badan_usaha !== 'Supplier Perorangan'}
                                                uploadConfig={{
                                                ...
                                                existingFile={supplier?.attachments?.find((a) => a.type === 'nib')}
                                            />
                                            {rules.is_nib && data.bentuk_badan_usaha !== 'Supplier Perorangan' && (
                                        ...
                                        <div className="w-full">
                                            <ResettableDropzone
                                                ...
                                                existingFile={supplier?.attachments?.find((a) => a.type === 'sppkp')}
                                            />
                                        ...
                                        <div className="w-full">
                                            <ResettableDropzone
                                                ...
                                                existingFile={supplier?.attachments?.find((a) => a.type === 'ktp')}
                                            />
            ...
            <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Konfirmasi Data Supplier</DialogTitle>
                        <DialogDescription>
