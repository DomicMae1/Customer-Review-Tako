...
import { Attachment, MasterSupplier } from '@/types';
...
export default function ViewSupplierForm({ supplier }: { supplier: MasterSupplier }) {
    ...
    const creatorId = supplier?.id_user;
    const currentUserId = props.auth.user?.id;
    const creatorRole = supplier?.creator_role || 'marketing';
    ...
    useEffect(() => {
        if (supplier?.id) {
            axios
                .get(`/supplier-status-check?supplier_id=${supplier.id}`)
                .then((res) => {
    ...
    useEffect(() => {
        const fetchManagerStatus = async () => {
            try {
                const res = await axios.get(`/perusahaan/${supplier.id_perusahaan}/has-manager`);
        ...
        fetchManagerStatus();
    }, [supplier.id_perusahaan, statusData?.submit_1_timestamps]);
    ...
    const handleSubmit = async (decision: 'approved' | 'rejected' | null = null) => {
        ...
        if (shouldCheckNpwpBeforeSubmit) {
            try {
                const res = await axios.post(route('supplier.check-npwp'), {
                    no_npwp: supplier.no_npwp, // Gunakan data dari prop supplier
        ...
        if (activeFile && activeFile.path.startsWith('temp/')) {
            try {
                ...
                const processRes = await axios.post('/supplier/process-attachment', {
                    path: activeFile.path,
                    nama_file: activeFile.nama_file,
                    // Gunakan ID Perusahaan dari supplier yang sedang dilihat
                    id_perusahaan: supplier.id_perusahaan,
                    mode: 'medium',
                    role: userRole,
                    type: fileType,
                    npwp_number: supplier.no_npwp, // Ambil NPWP dari data supplier yang sedang di-view
        ...
        if (supplier?.id_perusahaan) {
            formData.append('id_perusahaan', supplier.id_perusahaan.toString());
        }
        ...
        router.post('/submit-supplier-status', formData, {
            preserveScroll: true,
            preserveState: true,
            forceFormData: true,
            onSuccess: () => {
                ...
                setIsLoading(false);
                setTimeout(() => {
                    router.visit(`/supplier/${supplier.id}`, {
                        replace: true,
                        preserveState: false,
                    });
                }, 1300);
    ...
    return (
        <div className="rounded-2xl border-0 p-4">
            <h1 className="mb-4 text-3xl font-semibold">View Supplier</h1>

            <div className="space-y-6">
                {/* 🔹 Informasi Usaha */}
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <div>
                        <Label htmlFor="supplier_category">Kategori Supplier</Label>
                            <Input
                                id="supplier_category"
                                value={supplier.supplier_category || '-'}
                                disabled
                                className="bg-muted mt-1 block w-full rounded-md border px-3 py-2 text-sm shadow-sm"
                            />
                        </div>
                    ...
                    <div>
                        ...
                        <Input
                            id="bentuk_badan_usaha"
                            value={supplier.bentuk_badan_usaha}
                            disabled
                            className="w-full border border-gray-300 dark:border-neutral-600 dark:text-white"
                        />
                    </div>
                    <div>
                        ...
                        <Input
                            id="jenis_perusahaan"
                            value={supplier.jenis_perusahaan || '-'}
                            disabled
                            className="w-full border border-gray-300 dark:border-neutral-600 dark:text-white"
                        />
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div>
                        ...
                        <Textarea
                            id="alamat_penagihan"
                            value={supplier.alamat_penagihan}
                            disabled
                            className="h-20 w-full border border-gray-300 dark:border-neutral-600 dark:text-white"
                        />
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            value={supplier.status_perpajakan || '-'}
                            disabled
                            className="w-full border border-gray-300 dark:border-neutral-600 dark:text-white"
                        />
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div>
                        ...
                        <Input
                            id="jabatan_personal"
                            value={supplier.jabatan_personal || '-'}
                            disabled
                            className="w-full border border-gray-300 dark:border-neutral-600 dark:text-white"
                        />
                    </div>
                    <div>
                        ...
                        <Input
                            id="no_telp_personal"
                            value={supplier.no_telp_personal || '-'}
                            disabled
                            className="w-full border border-gray-300 dark:border-neutral-600 dark:text-white"
                        />
                    </div>
                    ...
                    {userRole === 'marketing' && (
                        <div className="mt-6 w-full md:w-1/3">
                            <ResettableDropzone
                                ...
                                uploadConfig={{
                                    url: '/supplier/upload-temp', // GANTI dengan endpoint upload Anda
                                    payload: {
                    ...
                    <div className="max-h-[90dvh] overflow-y-auto p-4 sm:p-6">
                        <DialogHeader className="space-y-2 text-left">
                            ...
                            <DialogDescription className="text-sm leading-relaxed">
                                Pastikan semua data supplier sudah benar. Setelah disubmit, data ini tidak bisa diedit kembali.
                            </DialogDescription>
                        </DialogHeader>
